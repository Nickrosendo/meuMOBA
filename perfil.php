...
$condicoes = "(`cod_anunciante` ='{$id_usu}')";
...
echo "<span class='label label-success'> Resultado: ".min($total, ($offset + $por_pagina))." de ".$total." anuncios</span> <br><br>";
...
if ($total > 0) {
  ...
  for ($n = 1; $n <= $paginas; $n++) {
    echo "<a class='texto_menu label label-success' href='perfil.php?index={$_GET['index']}&pagina={$n}'>{$n}</a>";
  }
  echo "</div>";
}
