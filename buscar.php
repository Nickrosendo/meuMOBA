...
$busca = urldecode($_GET['pesquisa']);
// ============================================
// Registros por página
$por_pagina = 15;
// Monta a consulta MySQL para saber quantos registros serão encontrados
$condicoes = "(`ativa` = 1) AND ((`titulo` LIKE '%{$busca}%') OR (`descricao` LIKE '%{$busca}%'))";
...
$sql = "SELECT * FROM `anuncios` WHERE {$condicoes} ORDER BY `premium` DESC , `preco` ASC LIMIT {$offset}, {$por_pagina}";
// Executa a consulta
$query = mysql_query($sql);
// ============================================
// Começa a exibição dos resultados
echo "<span class='label label-success'>".min($total, ($offset + $por_pagina))." de ".$total." resultados encontrados para '".$_GET['pesquisa']."' </span> <br><br>";
...
if ($total > 0) {
  ...
  for ($n = 1; $n <= $paginas; $n++) {
    echo "<a class='texto_menu label label-success' href='buscar.php?pesquisa={$_GET['pesquisa']}&pagina={$n}'>{$n}</a>";
  }
  echo "</div>";
}
